...
- `ERROR_ALERT_COOLDOWN_SECONDS`는 같은 종류의 오류를 텔레그램으로 반복 알리는 최소 간격입니다(아래 [텔레그램 알림](#텔레그램-알림) 참고).
- `LOG_DIR`는 날짜별 로그 파일(`trader_YYYYMMDD.log`)이 저장되는 디렉터리입니다. 화면에 출력되는 모든 로그가 동일하게 파일에도 남습니다.
...
`.env`의 `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`를 설정하면 아래 이벤트를 텔레그램으로 받습니다. 둘 중 하나라도 비어 있으면 전송을 조용히 건너뜁니다.

### 정상 매매 이벤트
- 데몬 시작
- 단기/중기 신규 매수(1차), 추가매수(2~5차), 익절, 장마감 손절/타임컷 청산, 리밸런싱
- 단기/중기 일일 브리핑(마감 루틴 종료 시 보유 현황 요약)

### 오류·이상 상황 알림
- 재시작 안전 차단(이전 실행에서 마감 루틴이 `started` 상태로 중단된 채 재시작됨)
- 포지션 대사 불일치(계좌와 장부 수량이 다름, 또는 같은 종목이 단기·중기 장부에 중복 등록됨)
- 거래일 확인·포지션 대사 조회 자체의 실패(네트워크 오류 등으로 확인을 못 한 경우, 자동 재시도 중)
- 키움 접근 토큰 발급 실패
- 주문 API 호출 실패, 부분체결, 미체결 타임아웃(지연 체결 감시열 등록)
- 일일 주문 상태 파일 또는 미체결 주문 상태 파일 오류(전체 자동 주문이 차단된 상태)
- 실시간 시세 웹소켓이 반복 재연결에 실패 중(백오프가 최대치에 도달한 경우)
- 장중 감시·마감 루틴에서 잔고 조회 실패, 개별 종목 처리 중 예외
- 그 외 데몬 루프에서 잡히지 않은 예외(마스터 데몬 예외)

같은 종류의 오류가 짧은 시간에 반복돼도 텔레그램이 도배되지 않도록, 오류 알림은 종류별로 `ERROR_ALERT_COOLDOWN_SECONDS`(기본 600초) 동안 한 번만 전송됩니다. 매매 이벤트(매수/매도/익절 등)와 확정된 위반(포지션 대사 불일치, 부분체결/미체결, 개별 종목 청산 오류 등)은 쿨다운 없이 매번 전송됩니다.
